// /functions/api/listKelas.js
// Endpoint: GET /api/listKelas
...
const GITHUB_API =
  `https://api.github.com/repos/${OWNER_REPO}/contents?ref=${encodeURIComponent(BRANCH)}`;
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ message: `Gagal fetch list kelas (${res.status})`, error }), {
    ...
    const kelasFiles = Array.isArray(data)
      ? data
          ...
          .sort() // opsional: urutkan alfabetis
      : [];
